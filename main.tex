...
\newcommand{\resumeProjectSectionHeading}[1]{\vspace{1mm}\item[] {\large\textbf{#1}}\vspace{-2.5mm}}
...
\resumeItemListStart
\item[] Scientific software engineering, reproducible computational workflows, astronomical data processing, geospatial analysis, high-performance computing, causal inference, machine learning for scientific data analysis, and large-scale data pipelines. Contributed to more than 30 scientific publications spanning astronomy, neuroimaging, psychiatry, and scientific software.
\resumeItemListEnd
\vspace{-2mm}
...
\item[$\bullet$] Implemented CCD calibration workflows, including bias, dark, and flat-field correction, astrometric calibration with Astrometry.net, and aperture and differential photometry for high-precision light curve generation.
...
\item[$\bullet$] Identified points in the node lifecycle where nodes were lost, implementing improvements that saved the company millions in revenue.
\resumeItemListEnd
...
\resumeItemListStart
\resumeProjectSectionHeading{Research Pipelines}
\resumeSubHeadingListStart
\resumeProject
...
{[Research Software]}
\resumeItemListStart
\item[$\bullet$] Developed a reproducible geospatial analysis pipeline integrating county- and tract-level social determinants of health with electronic health record data.
\item[$\bullet$] Automated geocoding quality control, FIPS recovery, census tract matching, and feature engineering using Python and external GIS APIs.
\item[$\bullet$] Supported publication-quality geospatial analyses and interactive visualizations for psychiatric epidemiology research.
\item[$\bullet$] Integrated Census tract and county-level indices (e.g., SVI, ADI) with electronic health record data to support reproducible spatial epidemiology analyses.
\resumeItemListEnd

\resumeProject
{Clinical Prediction Analysis Framework} 
{University of Minnesota Department of Psychiatry \& Behavioral Sciences} 
{2026} 
{[Research Software]}
\resumeItemListStart
\item[$\bullet$] Transformed an existing psychiatric prediction pipeline into a modular, reproducible research framework by expanding functionality, standardizing workflows, and improving usability in accordance with FAIR software principles.
\item[$\bullet$] Implemented automated repeated stratified cross-validation, LightGBM modeling, probability calibration, bootstrap/permutation robustness checks, SHAP interpretation, and publication-ready statistical reporting.
\item[$\bullet$] Supported multiple psychiatric prediction studies through reusable, publication-ready software workflows.
\resumeItemListEnd
...
{[Research Software]}
\resumeItemListStart
\item[$\bullet$] Built a reusable Python pipeline integrating multiple causal discovery algorithms, graph visualization, effect estimation, and reproducible statistical workflows for psychiatric EHR analyses.
\item[$\bullet$] Standardized preprocessing, graph generation, and downstream statistical analyses to support consistent comparison across causal learning methods.
\resumeItemListEnd
...
\resumeProjectSectionHeading{Research Infrastructure}
\resumeSubHeadingListStart
\resumeProject
...
\resumeProjectSectionHeading{Independent Projects}
\resumeSubHeadingListStart
\resumeProject
{Waste Not} 
...
{[Independent Project]}
\resumeItemListStart
\item[$\bullet$] Designed and developed a sustainability application using a nationwide, self-curated recycling database created through large-scale web scraping, data normalization, and categorization.
\resumeItemListEnd
...
\vspace{0.7mm}
\begin{justify}\begin{itemize}[leftmargin=3ex, rightmargin=2ex, labelsep=1.2mm, itemsep=2.2mm, topsep=1.5mm]\small
\item[]\textbf{Programming:} Python, Bash, UNIX/Linux, Git/GitHub, LaTeX, Docker, Singularity, SQL
\item[]\textbf{Infrastructure \& HPC:} AWS (S3), Ceph, SLURM, Kubernetes, batch processing, data validation, Globus, Grafana, Prometheus, HPC workflow design
\item[]\textbf{Machine Learning \& Statistics:} logistic regression, predictive modeling, LightGBM, calibration (Platt/isotonic), repeated stratified cross-validation, bootstrap/permutation inference, FDR correction (Benjamini-Hochberg), SHAP interpretability, ROC-AUC/PR-AUC/Brier metrics, causal inference \& discovery
\item[]\textbf{Scientific Computing:} neuroimaging/MRI segmentation, astronomical image processing (FITS, Astrometry.net, Astropy, Photutils), CCD calibration, differential photometry, time-domain imaging, deep learning inference workloads and compute planning, GIS analysis, QGIS visualization, geocoding/FIPS recovery
\item[]\textbf{Data Engineering:} ETL pipelines, EHR integration, census tract and county-level feature engineering, data harmonization, reproducible research workflows, workflow automation
\item[]\textbf{Collaboration:} Jira, Linear, Kanban, Agile/Scrum, SOP documentation, Slack, Microsoft Teams, Google Workspace, Box, Salesforce API automation, compliant web scraping
...
\begin{justify}\begin{itemize}[leftmargin=3ex, rightmargin=2ex, labelsep=1.2mm, itemsep=2.5mm, topsep=2mm]\small
