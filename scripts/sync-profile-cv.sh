...
# Sync the generated CV PDF from this repository to a target GitHub repo.
...
TARGET_FILE_PATH="${TARGET_FILE_PATH:-AudreyHoughton_CV.pdf}"
...
print_usage() {
  cat <<'EOF'
Usage:
  scripts/sync-profile-cv.sh

Optional environment variables:
  TARGET_REPO_URL   Git URL for destination repository
  TARGET_BRANCH     Branch to push to (default: main)
  TARGET_FILE_PATH  Path inside destination repo for PDF (default: AudreyHoughton_CV.pdf)
  COMMIT_MESSAGE    Commit message for sync commit
  BUILD_CV          1 to run latex build first, 0 to skip (default: 0)
...
if [[ "${BUILD_CV}" == "1" ]]; then
  echo "Building CV PDF from ${SOURCE_TEX}..."
...
echo "CV synced to ${TARGET_REPO_URL}:${TARGET_FILE_PATH}"
